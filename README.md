...
Once a route is found, **+ Add a paper** appears: search for a third paper and the app searches from it to both papers already on screen, and draws all three pairwise routes. Keeping only the two shortest would connect all three papers with fewer lines, but it draws the two papers whose route was dropped as though they only meet through the third — and when the three distances tie, which one gets dropped comes down to the order the papers were entered. Every pair is searched, so every pair is shown; each cluster shrinks from 16 papers to about 11 so the three-cluster view stays as readable as the two-paper one, and the third search reuses everything already fetched for the first two so it typically costs few or no extra requests. Press the **✕** beside the third search box to drop it and return to a two-paper bridge.
